...
def main():
    ...
    mensagens = StayBox_funcoes.getFilaEnvio('9', b) #3 = aviso bloqueio movel
    b.close()
    ...
    if mensagens != None: #TEM MENSAGENS PARA ENVIAR
      ...
      if result == None:
        logging.error('AVISO COBRANÇA MOVEL - main() - getLogin=NONE')
        sys.exit('AVISO COBRANÇA MOVEL - main() - getLogin=NONE')
        exit
      else:
        ...
        result = SZChat_funcoes.getMensagemConfig(9)
        if result == None:
          logging.error('AVISO BLOQUEIO MOVEL- main() - getMensagem=NONE')
          sys.exit('AVISO BLOQUEIO MOVEL - main() - getMensagem=NONE')
        else:
        ...
        if auth.status_code != 200:
          logging.error('AVISO BLOQUEIO MOVEL - Não foi possível autenticar na API.')     
          sys.exit('AVISO BLOQUEIO MOVEL - Não foi possível autenticar na API.')
        else:
            jauth = auth.json()
            vTOKEN_APP = jauth['token']
            vATENDDANCE_ID = SZChat_funcoes.getAtenddanceID(2) #FINANCEIRO Cobrança
        ...
        for m in  mensagens:
          ...
          if vSend: #se está em horário comercial
            ...
            StayBox_funcoes.setLogEnvio('9', m['contract_id'], m['client_id'], m['nome'], vCelular, str(send), m['frt_id'], b)
